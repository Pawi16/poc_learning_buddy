...
class QuizQuestion(BaseModel):
    ...
    question_text: str = Field(..., description="The main question text. Must be self-contained (no 'as seen above').")
    ...
    explanation: str = Field(..., description="Technical explanation of the answer (principles, not just 'source says so')")
...
def build_type_instructions(question_types: List[str], difficulty: str) -> str:
    ...
    if "statement_verification" in question_types:
        # ✅ FIX: Added stmt_logic here so it respects difficulty
        instructions.append(f"""
...
def generate_quiz_for_topic(payload: TopicPayload):
    ...
    for config in payload.quiz_config:
        ...
        - You are generating {total_questions} questions.
        - EACH question must test a DIFFERENT concept, API method, or section of the text.
        - DO NOT generate duplicates or questions that ask the same thing in different words.
        
        ⛔ SELF-CONTAINED RULE:
        - Questions must be standalone.
        - BAD: "In the example above...", "As mentioned in the text..."
        - GOOD: "In the Builder pattern...", "When using the Telescoping Constructor pattern..."
        - If referring to a specific code example (like NutritionFacts), mention it by name, do not just say "the example".
        
        ✅ EXPLANATION RULE:
        - Explanations must explain the *principle*.
        - BAD: "Because the source material says so."
        - GOOD: "Because the JavaBeans pattern allows the object to be in an inconsistent state during construction..."
      ...
      "quiz_config": [
        {
          "difficulty": "medium",
          "quiz_type_config": [
            { "type": "normal_multiple", "number": 6 }, # Increased number to test duplication check
            { "type": "statement_verification", "number": 4 }
          ]
        }
      ]
    }
