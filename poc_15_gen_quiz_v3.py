...
# Intermediate Step: Extracted Fact
class KeyConcept(BaseModel):
    concept_id: int
    fact_content: str = Field(..., description="A single, atomic fact or rule from the text.")
    source_segment: str = Field(..., description="The specific sentence from text this fact comes from.")

class ConceptList(BaseModel):
    concepts: List[KeyConcept]

# Final Step: Quiz Question
...
class QuizQuestion(BaseModel):
    ...
    distractor_analysis: str = Field(..., description="Explain why the wrong options are plausible but incorrect.")
    explanation: str = Field(..., description="The correct answer explanation.")

class GeneratedQuiz(BaseModel):
    questions: List[QuizQuestion]

# --- 3. HELPER: DYNAMIC INSTRUCTION BUILDER (Imported from Old Code) ---
def build_single_type_instruction(q_type: str, difficulty: str) -> str:
    """
    Returns specific formatting rules for ONE question type.
    """
    instructions = ""
    ...
    # 2. Build Specific Instruction
    if q_type == "normal_multiple":
        instructions = f"""
        [TYPE RULES: Normal Multiple Choice]
        - Complexity: {difficulty.upper()}
        - Goal: Focus on {focus}
        - Options: Provide 4 options (A, B, C, D). {distractors}
        - Template: Question text ending with '?'
        """

    elif q_type == "statement_verification":
        instructions = f"""
        [TYPE RULES: Statement Verification]
        - Complexity: {difficulty.upper()}
        - Format: The question text must be exactly: "Which of the following statements is TRUE regarding [Topic]?"
        - Options: 4 distinct statements. Only one is true.
        - Logic: {stmt_logic}
        """

    elif q_type == "statement_counting":
        instructions = f"""
        [TYPE RULES: Statement Counting]
        - Complexity: {difficulty.upper()}
...
def step_1_extract_concepts(text: str, count: int) -> List[KeyConcept]:
    """Extracts atomic facts."""
    print(f"   🔍 Phase 1: Extracting {count} unique concepts...")
    
    prompt = f"""
    Analyze the following text and extract exactly {count} distinct, atomic facts or rules.
    
    SOURCE TEXT:
    {text}
    
    RULES:
    1. Each fact must be independent.
    2. Cover different parts of the text (Definitions, Pros/Cons, Code Rules).
    3. Do NOT repeat information.
    
    Return a JSON object matching 'ConceptList'.
    """
    
    try:
        result = llm.with_structured_output(ConceptList).invoke(prompt)
        return result.concepts
    except Exception as e:
        print(f"Extraction failed: {e}")
        return []

def step_2_generate_questions(concepts: List[KeyConcept], config_type: str, difficulty: str) -> List[QuizQuestion]:
    """Converts specific facts into questions using STRICT formatting."""
    print(f"   ✍️ Phase 2: Converting concepts to '{config_type}' questions ({difficulty})...")
    
    # 1. Get the Specific Formatting Rules
    formatting_instructions = build_single_type_instruction(config_type, difficulty)
    
    # 2. Prepare Fact String
    facts_text = "\n".join([f"{c.concept_id}. {c.fact_content}" for c in concepts])
    
    prompt = f"""
    You are an Expert Exam Creator. 
    Transform the provided FACT LIST into {len(concepts)} high-quality '{config_type}' questions.
    
    DIFFICULTY: {difficulty.upper()}
    
    INPUT FACTS (One question per fact):
    {facts_text}
    
    TASK RULES:
    1. Create exactly ONE question for each Fact provided above.
    2. Type Constraint: All questions must be of type '{config_type}'.
    
    👇 STRICT FORMATTING RULES (FOLLOW THESE EXACTLY) 👇
    {formatting_instructions}
    
    OUTPUT:
    Return a JSON object matching 'GeneratedQuiz'.
    """
    
    try:
        result = llm.with_structured_output(GeneratedQuiz).invoke(prompt)
...
def generate_robust_quiz(payload):
    print(f"🚀 Starting Pipeline for: {payload['topicName']}")
    
    final_questions = []
    current_id_counter = 1
    
    for quiz_cfg in payload['quiz_config']:
        difficulty = quiz_cfg['difficulty']
        
        for type_cfg in quiz_cfg['quiz_type_config']:
            q_type = type_cfg['type']
            q_count = type_cfg['number']
            
            # STEP 1: Extract
            concepts = step_1_extract_concepts(payload['topicContent'], count=q_count)
            
            if not concepts:
                continue

            # STEP 2: Generate (Now with strict formatting)
            questions = step_2_generate_questions(concepts, q_type, difficulty)
            
            for q in questions:
                q.id = current_id_counter
                q.type = q_type 
                current_id_counter += 1
...
if __name__ == "__main__":
    ...
    questions = generate_robust_quiz(request_payload)

    print("\n" + "="*50)
    print("FINAL QUIZ OUTPUT")
    print("="*50)
    for q in questions:
        print(f"\nQ{q.id} [{q.type}] {q.question_text}")
        for opt in q.options:
            print(f"  {opt.id}) {opt.text}")
        print(f"✅ Correct: {q.correct_option_id}")
        print(f"🧠 Distractor Analysis: {q.distractor_analysis}")
