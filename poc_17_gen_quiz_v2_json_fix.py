...
class GeneratedQuizBatch(BaseModel):
    """ Intermediate model for a single batch """
...
# --- 3. DYNAMIC PROMPT BUILDER (RESTORED) ---
...
# --- 4. GENERATION LOOP (MODERN FIX) ---
def generate_quiz_for_topic(payload: TopicPayload):
    print(f"🚀 Processing Topic: {payload.topicName}")
    final_results = []

    # ✅ MODERN FIX: Use JsonOutputParser
    # This automatically strips markdown code blocks (```json) for us.
    parser = JsonOutputParser(pydantic_object=GeneratedQuizBatch)
    ...
    for config in payload.quiz_config:
        ...
        for type_req in config.quiz_type_config:
            ...
            for attempt in range(max_retries + 1):
                try:
                    # ✅ Invoke the Chain
                    # The parser handles the markdown cleaning and returns a Dict
                    raw_result_dict = chain.invoke(prompt)
                    
                    # Validate with Pydantic
                    batch_result = GeneratedQuizBatch(**raw_result_dict)
        ...
        final_results.append(final_quiz.model_dump())
